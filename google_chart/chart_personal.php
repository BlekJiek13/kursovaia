...
$course_on_users_orders =  user_costs_by_category($_SESSION['id_users']);
?>
	...
	<script>
		...
		function drawChart() {
			var data = new google.visualization.DataTable(
				...
				"rows": [';
				foreach($course_on_users_orders as $courses){
					$data.= '{"c":[{"v":"';
						$data.=  $courses['name_category'];
						$data.=' "},{"v":';
						$data.= $courses['Sum'];
						$data.= '  }]},';
				}
			...
			// Set chart options
			var options = {
				'title': 'График затрат пользователя по категориям',
				'width': 1100,
				'height': 800,
				'pieHole': 0.5,
				// 'slices': {
